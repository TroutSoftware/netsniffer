...
#include "serializer_raw.h"

namespace serializer_raw {
namespace {

static const char *s_name = "serializer_raw";
static const char *s_help = "Serializes the raw data part of LioLi trees";
    ...
    std::string serialize(const LioLi::Tree &&tree) override {
      ...
      if (first_write) {
        if (settings.secret.size() != 9) {
          snort::ErrorMessage("ERROR: RAW secret not set to a valid value\n");
          {
...
class Module : public snort::Module {
  ...
  bool set(const char *, snort::Value &val, snort::SnortConfig *) override {
    if (val.is("secret_sequence")) {
      ...
      if (input.length() != 18) {
        snort::ErrorMessage("ERROR: option >secret_sequence< needs 9 "
      ...
      if (9 != sscanf(input.c_str(),
        ...
        snort::ErrorMessage("ERROR: option >secret_sequence< could not "
