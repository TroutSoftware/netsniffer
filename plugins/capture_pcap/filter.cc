...
Filter::Filter(std::string &&filter_string, std::shared_ptr<Settings> settings, PegCounts &pegs) : settings(settings), pegs(pegs), filter_string(filter_string) {
  compile();
}
...
void Filter::compile() {
  ...
  if(pcap_compile(dead, &compiled, filter_string.c_str(), settings->optimize_filter, PCAP_NETMASK_UNKNOWN)) {
    compiled_valid = false;
...
bool Filter::match(snort::Packet *p) {
  ...
  if (bpf_filter(compiled.bf_insns, filter_pkt, filter_pkt_len, filter_pkth_len)) {
    pegs.pkg_matched++;
    return true;
  }
