...
template <class T>
concept PegsTypeConcept = TypeConcept<T> &&
                      requires {
                        ...
                        {
                          T::get_type()
                        } -> std::same_as<CountType>;
                        
                      } && std::constructible_from<T, PegCount&>;

template <class T> struct CheckIsPegsType : std::bool_constant<PegsTypeConcept<T>> {};
