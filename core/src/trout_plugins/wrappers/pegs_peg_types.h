...
enum class PegType {
  // END,   // sentinel value
  SUM = CountType::SUM, // running total: tracks cumulative total number of
                        // items seen (eg #events)
  NOW = CountType::NOW, // current level: gives snapshot of current number of
                        // items (eg current #sessions)
  MAX = CountType::MAX, // maximum level: tracks maximum value seen (eg max
                        // #sessions)
};
...
template <PegType type> class PegBaseType : public GenericTypeBaseClass {
public:
...
template <PegType type> class Type<type> : public PegBaseType<type> {
...
template <> class Type<PegType::SUM> : public PegBaseType<PegType::SUM> {
  PegCount &count;
...
template <> class Type<PegType::MAX> : public PegBaseType<PegType::MAX> {
  PegCount &count;
...
template <> class Type<PegType::NOW> : public PegBaseType<PegType::NOW> {
  PegCount &count;

public:
  Type(PegCount &count) : count(count) {}

  void set(PegCount value) { count = value; }
};
