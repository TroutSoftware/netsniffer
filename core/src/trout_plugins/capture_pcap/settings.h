...
struct Settings : std::enable_shared_from_this<Settings> {
  ...
  struct MapItem {
    ...
    std::shared_ptr<PcapDumper>
        dumper; // Multiple filters can dump to the same pcap
    std::optional<uint16_t> port;
    std::optional<uint32_t> ip;
  };

  std::list<std::unique_ptr<MapItem>>
      map; // This might be made more inteligent at a later time
