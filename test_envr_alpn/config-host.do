...
[ -f /etc/apparmor.d/usr.bin.bwrap ]  || (\
  echo "Please copy the file 'bwrap' from $(pwd) to /etc/apparmor.d/usr.bin.bwrap and restart apparmor 'sudo /etc/init.d/apparmor restart'" >&2 \
  && exit 1)

[ -f /etc/apparmor.d/usr.bin.unshare ]  || (\
  echo "Please copy the file 'usr.bin.unshare' from $(pwd) to /etc/apparmor.d/usr.bin.unshare and restart apparmor 'sudo /etc/init.d/apparmor restart'" >&2 \
  && exit 1)
